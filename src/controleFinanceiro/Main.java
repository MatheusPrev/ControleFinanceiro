...
public class Main {
	...
	static Banco Nubanc = new Banco("Nubanc",9831887,7013524);
	static Banco[] bancos = {Itau,Nubanc};
	...
	public static void main(String[] args) {
		...
		Bens NubancCC = new Bens(Nubanc,"Conta corrente",107.05);
		Bens NubancCredito = new Bens(Nubanc,"Credito",-51.14);
		Bens[] patrimonio = {ItauCC,ItauCredito,ItauPrevidencia,NubancCC,NubancCredito};
	...
	static public int menu(Bens[] patrimonio) {	
		...
		label1.setText("Total: "+ valorTotal);
		//ImageIcon image = new ImageIcon("C:\\Users\\mathe\\eclipse-workspace\\ControleFinanceiro\\midia\\icone1.png");
		//label.setIcon(image);
		//label.setHorizontalAlignment(JLabel.LEFT);
		//label.setVerticalAlignment(JLabel.TOP);
		label1.setForeground(Color.green);
		label1.setFont(new Font("Arial",Font.BOLD,30));
		label1.setBounds(7,7,210,30);
		...
		for(int i=0;i<bancos.length;i++) {
			...
			tituloBanco[i].setFont(new Font("Arial",Font.BOLD,18));
		}
		for(int i=0;i<patrimonio.length;i++) {
			...
			//tipoMenu[i].setBounds(7,7,200,20);
			//valoresMenu[i].setBounds(7,7,200,20);
			//valoresMenu[i].setHorizontalAlignment(JLabel.LEFT);
		}
		
		JPanel total = new JPanel();
		total.setBounds(5,25,250,40);
